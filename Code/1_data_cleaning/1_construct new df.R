...
# Script: 1_construct new df.R
# Author: Nemo Zhou
# Date started: Unknown (pre-existing script before documentation standard was applied)
# Date last updated: 2026-06-28
# Purpose: Constructs an initial longitudinal working dataset from HPFS source data, including participant identifiers, cycle structure, passthrough variables, and mapped questionnaire variables used to build frailty deficits.
# ==============================================================================
...
disease_maps <- list(
  ...
  hfra = list(hp86="ost86", hp88="hfra88", hp90="hfra90", hp92="hfra92", hp94="hfra94", hp96="hfra96", hp98="hfra98", hp00="hfra00", hp02="hfra02", hp04="hfra04", hp06="hfra06", hp08="hfra08", hp10="hfra10", hp12="hipf12", hp14="hipf14", hp16="hipf16", hp18="hipf18", hp20="hipf20"),
  ...
  dvrt = list(hp86=character(0), hp88=character(0), hp90="dvrt90", hp92="dvrt92", hp94="dvrt94", hp96="dvrt96", hp98="dvrt98", hp00="dvrt00", hp02="dvrt02", hp04="dvrt04", hp06="dvrt06", hp08="dvrt08", hp10="dvrt10", hp12="dvrt12", hp14=character(0), hp16="dvrt16", hp18=character(0), hp20="diver20"),
...
for (cycle in all_cycles) {
  ...
  # ----------------------------------------------------------------------
  # PATCH 2: FIX DIVERTICULITIS CODING IN 2020 (diver20: 1=No, 2=Yes)
  # ----------------------------------------------------------------------
  if (cycle == "hp20" && "diver20" %in% names(df)) {
    df$diver20 <- case_when(
      df$diver20 == 2 ~ 1.0, # Yes
      df$diver20 == 1 ~ 0.0, # No
      TRUE ~ NA_real_
    )
  }
  ...
  for (c_type in names(disease_maps)) {
    ...
    if (length(vars_present) > 0) {
      ...
      df[[new_var_name]] <- case_when(
        ...
        has_disease ~ 1.0,
        TRUE ~ 0.0    # <-- New Logic: Blanks without passthrough = 0
      )
    } else {
  ...
  for (cat_name in names(poly_map)) {
    ...
    if (length(cat_vars_present) > 0) {
      ...
      poly_flags[[cat_name]] <- case_when(
        ...
        has_med ~ 1L,
        TRUE ~ 0L      # <-- New Logic: Blanks without passthrough = 0
      )
    } else {
...
expected_weights <- paste0("weight_", c("86", "88", "90", "92", "94", "96", "98", "00", "02", "04", "06", "08", "10", "12", "14", "16", "18", "20"))
...
target_years <- c("88", "90", "92", "94", "96", "98", "00", "02", "04", "06", "08", "10", "12", "14", "16", "18", "20")
prev_years   <- c("86", "88", "90", "92", "94", "96", "98", "00", "02", "04", "06", "08", "10", "12", "14", "16", "18")
