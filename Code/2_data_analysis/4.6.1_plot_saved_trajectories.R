...
# Script:  4.6.1_plot_saved_trajectories.R
# Author:  Nemo Zhou
# Date started:      2026-07-20
# Date last updated: 2026-07-22 (renamed from 4.1_plot_saved_trajectories.R)
#
# Purpose:
#   Create trajectory plots from the saved 4.1 prediction CSV without fitting
#   any models. The plot includes M0 raw spline, M1 primary spline, M2 full
#   spline, and M3 matching-set spline predictions.
#
# Inputs:
#   Results/cancer/data/4.1_spline_predicted_trajectories.csv
#   Results/cancer/data/4.1_spline_model_status.csv
#
# Outputs:
#   Results/cancer/visuals/4.1_saved_trajectories_M0_M1_M2_M3.pdf
#   Results/cancer/visuals/4.1_saved_trajectories_M0_M1_M2_M3.html
#
...
prediction_path <- file.path(data_dir, "4.1_spline_predicted_trajectories.csv")
status_path <- file.path(data_dir, "4.1_spline_model_status.csv")
...
if (length(missing_models)) {
  ...
  stop(
    ...
    "The saved prediction file is missing one or more renumbered spline models. ",
    "Rerun ",
    "Code/2_data_analysis/4.1_GLME_high_low_burden.R, then rerun this plotting script.",
    call. = FALSE
  )
}
...
if (file.info(status_path)$mtime > file.info(prediction_path)$mtime) {
  warning(
    ...
    "prediction CSV was regenerated from the same 4.1 run before interpreting the plot.",
    call. = FALSE
  )
}
...
p_trajectory <- ggplot(
  ...
  labs(
    title = "Saved frailty trajectories: M0 and M1-M3",
...
pdf_path <- file.path(visuals_dir, "4.1_saved_trajectories_M0_M1_M2_M3.pdf")
...
html_path <- file.path(visuals_dir, "4.1_saved_trajectories_M0_M1_M2_M3.html")
html_text <- c(
  "<!doctype html>",
  "<html lang=\"en\">",
  "<head><meta charset=\"utf-8\"><title>4.1 Saved Trajectories M0 M1 M2 M3</title></head>",
  "<body>",
  svg_markup,
  "</body></html>"
)
